...
export class TelaInicialComponent implements OnInit {
    ...
    transacoesCategorias: { [key: string]: number } = {};
    metasCategorias: { [key: string]: number } = {}; // Armazena metas por categoria
    chartsData: any[] = [];
    ...
    carregarCategorias(): void {
        this.categoriaService.getCategoriasPorUsuarioId('DESPESA').subscribe({
            next: (categorias: Categoria[]) => {
                this.categoriasDespesa = categorias;
                this.buscarMetas(categorias);
    ...
    buscarMetas(categorias: Categoria[]): void {
        categorias.forEach((categoria) => {

            this.dashboardService.getMetas(categoria.id).subscribe({
                next: (metas: MetasFinanceiras[]) => {
                    const metaCategoria = metas.find((meta) => meta.categoriaId === categoria.id); // Comparando com categoriaId
                    if (metaCategoria) {
                        this.metasCategorias[categoria.id] = metaCategoria.valor;
                    } else {
                        this.metasCategorias[categoria.id] = 0;
                    }
                    this.renderCategoryCharts();
                },
                error: (error) => {
                    console.error('Erro ao buscar metas para a categoria', categoria.id, error);
                }
            });
        });
    }
    ...
    buscarTransacoes(): void {
        ...
        const dataFim = moment().endOf('month').format('YYYY-MM-DD');

        this.dashboardService.getTransacoes(dataInicio, dataFim).subscribe({
            next: (transacoes) => {
                // Agora, vamos somar as transações por categoria
                transacoes.forEach((transacao) => {
                    const categoriaId = transacao.categoriaId;
                    if (this.transacoesCategorias[categoriaId]) {
                        this.transacoesCategorias[categoriaId] += parseFloat(transacao.valor);
                    } else {
                        this.transacoesCategorias[categoriaId] = parseFloat(transacao.valor);
                    }
                });
    ...
    renderCategoryCharts() {
        const categories = this.categoriasDespesa.map((categoria) => {
            const transacoesCategoria = this.transacoesCategorias[categoria.id] || 0;
            const metaCategoria = this.metasCategorias[categoria.id] || 0;

            const percentualRealizado = metaCategoria ? Math.min((transacoesCategoria / metaCategoria) * 100, 100) : 0;

            return {
                name: categoria.nome,
                percentage: percentualRealizado
            };
        });
